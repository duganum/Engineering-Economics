...
# 3. Load Engineering Economics Data
@st.cache_data
        ...
        if st.button("🚀 Submit Session", use_container_width=True):
            if "chat_session" in st.session_state:
                ...
                with st.spinner("Analyzing mastery..."):
                    analyze_and_send_report(st.session_state.user_name, f"EngEco: {prob['id']}", f"History:\n{history_text}\nFeedback: {feedback}")
...
# --- Page 3: Interactive Lecture ---
        ...
        if st.button("🚀 Submit Lecture Session", use_container_width=True):
            if "lec_session" in st.session_state:
                ...
                with st.spinner("Submitting..."):
                    analyze_and_send_report(st.session_state.user_name, f"EngEco LECTURE: {topic}", f"History:\n{history_text}\nFeedback: {lec_feedback}")
    ...
    with col_tutor:
        ...
        if "lec_session" not in st.session_state:
            try:
                model = get_gemini_model(f"You are Prof. Um teaching {topic}. Engage with Socratic questions.")
                st.session_state.lec_session = model.start_chat(history=[])
                greeting = f"Welcome! What part of {topic} shall we start with?"
        ...
        if lec_input := st.chat_input("Ask a question about the concept..."):
            ...
            try:
                ...
                st.session_state.api_busy = False
                st.error("Rate Limit reached. Please wait.")
            except Exception as e:
                st.session_state.api_busy = False
                st.error(f"Error: {e}")
